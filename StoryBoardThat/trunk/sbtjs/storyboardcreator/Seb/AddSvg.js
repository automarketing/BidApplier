...
var AddSvg = function ()
{
    ...
    AddSvgObject.AddSvgToTabs = function (node, width, height, content, isSceneItem, onitemadded)
    {
        ...
        canvas_obj.id = 'canvas_' + this.CurrentShapeIndex;
        canvg(canvas_obj, $('.ClipartLibrary')[0].outerHTML);
        canvas_obj.draggable=true;
        $('#' + parentId).append(canvas_obj); 
        
        canvas_obj.addEventListener("dragstart", function(ev) {
            ev.dataTransfer.setData("text", ev.target.id);
            ev.dataTransfer.setData("isSceneItem", isSceneItem); 
        });
 
        SVGBUF[ canvas_obj.id ] = { xml : $('.ClipartLibrary').children().eq(0).detach() , isSceneItem : isSceneItem };

        $('.ClipartLibrary').remove();
    ...
    AddSvgObject.AddSvgToStoryboard = function (shape, x, y, addUndo)
    {
        try
        {
            ...
            //StoryboardContainer.UpdateGradientIds(outerSvg);
