...
(function ($)
{
    ...
    var proxyHandlers = {
        ...
        handleTouchStart: function (e)
        {
            ...
            if ($target)
            {
                ...
                if (event.touches.length === settings.requiredTouches)
                {
                    switch (eventType)
                    {
                        ...
                        case 'pinch':
                            ...
                            if (points)
                            {
                                ...
                                data.startXOffset = points.x2 - points.x1; // SOC 2014-01-05
                                data.startYOffset = points.y2 - points.y1; // SOC 2014-01-05

                                // Trigger added SOC 2014-01-05
                                $target.trigger('touchy-pinch', ['start', $target, {
                                    "scale": 1,
                                    "previousScale": 1,
                                    "currentPoint": data.startPoint,
                                    "startPoint": data.startPoint,
                                    "startDistance": data.startDistance,
                                    "x1": points.x1,        //Added by ABS 6/10/13
                                    "x2": points.x2,
                                    "y1": points.y1,
                                    "y2": points.y2,
                                    "startXOffset": data.startXOffset, // SOC 2014-01-05
                                    "startYOffset": data.startYOffset,
                                    "currentXOffset": data.startXOffset,
                                    "currentYOffset": data.startYOffset
                                }]);
                            }
                            break;
            ...
            if ($target)
            {
                ...
                if (event.touches.length === settings.requiredTouches)
                {
                    ...
                    switch (eventType)
                    {
                        ...
                        //////////////// DRAG ////////////////
                        case 'drag':
                            if (event.touches.length === data.settings.requiredTouches) // ABS - added event.
                            {
                                updateDragData(data, touches, e.timeStamp);
                        ...
                        case 'pinch':
                            ...
                            if (points)
                            {
                                ...
                                data.previousXOffset = data.currentXOffset || data.startXOffset; // SOC 2014-01-05
                                data.previousYOffset = data.currentYOffset || data.startYOffset; // SOC 2014-01-05
                                data.currentXOffset = points.x2 - points.x1; // SOC 2014-01-05
                                data.currentYOffset = points.y2 - points.y1; // SOC 2014-01-05
                                if (GestureChangeIsEmpty())
                                {
                                    ...
                                    if (currentDistance > settings.pxThresh)
                                    {
                                        $target.trigger('touchy-pinch', ['move', $target, {
                                            ...
                                            "startXOffset": data.startXOffset, // SOC 2014-01-05
                                            "startYOffset": data.startYOffset,
                                            "currentXOffset": data.currentXOffset,
                                            "currentYOffset": data.currentYOffset
                                        }]);
                                    }
                                }
                            }
    ...
    // other private methods
      ...
      //ABS 6/20/13 - has gesture change doesn't work correctly... Safari returns null, andorid returns undefined.. very different behavior
      GestureChangeIsEmpty = function ()
      {
          if (window.ongesturechange == null)
              return true;
      ...
      getTwoTouchPointData = function (e)
      { // could become multitouch point data for any number of touches?
          var points = false,
